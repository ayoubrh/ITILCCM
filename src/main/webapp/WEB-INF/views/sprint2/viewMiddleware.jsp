...
<head>
	...
	<title>Détail Middleware - ITIL-CCM</title>
...
-->
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title col-md-9">Détail Middleware</span>
						<s:authorize ifAnyGranted="ROLE_ADMIN">
						<a href="<c:url value="/config/admin/edit/middleware?id=${middleware.id }"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Modifier</a>
						<a href="<c:url value="/config/admin/delete/middleware?id=${middleware.id }"/>" class="btn btn-danger"><i class="fa"></i>&nbsp;Supprimer</a>
						</s:authorize>
						<br>
					</div>
							...
							<div class="tab-content tab-content-bordered panel-padding">
								<div class="widget-article-comments tab-pane panel no-padding no-border fade in active" id="profile-tabs-proprietes">
									...
									<div class="form-group">
										...
										${middleware.criticite }
									</div>
									
									<div class="form-group">
										<label for="jq-validation-marque" class="col-sm-3 control-label">Système : </label>
										<c:if test="${middleware.serveur.id != null }">
										<a href="<c:url value="/config/view/serveur?id=${middleware.serveur.id }" />">${middleware.serveur.nom }</a>
										</c:if>
										<c:if test="${middleware.machineVirtuelle.id != null }">
										<a href="<c:url value="/config/view/machineVirtuelle?id=${middleware.machineVirtuelle.id }" />">${middleware.machineVirtuelle.nom }</a>
										</c:if>
									...
									<div class="form-group">
										...
										<a href="<c:url value="/config/view/licenceLogiciel?id=${middleware.licenseLogiciel.id }" />">${middleware.licenseLogiciel.nom }</a>
									</div>
									<div class="form-group">
										<label for="jq-validation-marque" class="col-sm-3 control-label">Chemin d'installation: </label>
										${middleware.cheminD_installation }
									</div>
									
									<div class="form-group">
										<label for="jq-validation-modele" class="col-sm-3 control-label">Date de mise en production: </label>
										<fmt:formatDate type="date" dateStyle="long" value="${middleware.dateDeMiseEnProduction }" />
									</div>
								...
								<div class="tab-pane fade widget-tickets" id="profile-tabs-tickets">
										...
										<div class="table-primary">
											<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="jq-datatables-example-tickets">
												...
												<tbody>
												<c:forEach items="${middleware.ticketsIncident}" var="ticket">
