...
<head>
	...
	<title>Recherche Périphériques - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Périphériques</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/per"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Périphérique</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="per" class="search-form bg-primary">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/per?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/pc?id=${ci.ordinateur.id }" />">${ci.ordinateur.nom }</a></td>
											<td>${ci.statut }</td>
											<td>${ci.marque }</td>
											<td>${ci.modele }</td>
