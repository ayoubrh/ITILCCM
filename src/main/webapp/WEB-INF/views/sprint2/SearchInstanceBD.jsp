...
<head>
	...
	<title>Recherche Instance de base de données- ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche d'Instance de base de données</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/instanceBD"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Instance BD</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="instanceBD" class="search-form bg-primary">
				<div class="input-group input-group-lg">
					<span class="input-group-addon no-background"><i class="fa fa-search"></i></span>
					<input type="text" name="ibd" class="form-control" value="${instanceBD }" placeholder="Entrez le nom à chercher...">
				...
				<!-- Users search -->
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								<thead>
									<tr>
										<th>Nom</th>
										<th>Serveur de base de données</th>
										<th>Criticité</th>
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/instanceBD?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/serveurBD?id=${ci.serveurDeBasseDeDonnees.id }" />">${ci.serveurDeBasseDeDonnees.nom}</a></td>
											<td>${ci.criticite }</td>
