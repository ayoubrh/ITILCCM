...
<head>
	...
	<title>Recherche Licence OS - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Licences OS</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/licenseOs"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Licence OS</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="licenseOs" class="search-form bg-primary">
				<div class="input-group input-group-lg">
					<span class="input-group-addon no-background"><i class="fa fa-search"></i></span>
					<input type="text" name="lo" class="form-control" value="${licenseOs }" placeholder="Entrez le nom à rechercher...">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/licenseOs?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/versionOs?id=${ci.versionOs.id }" />">${ci.versionOs.nom }</a></td>
											...
											<td><fmt:formatDate type="date" dateStyle="long" value="${ci.dateDeFinDeValiite }" /></td>
											<td>${ci.cle }</td>
