...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Modification Machine virtuelle - ITIL-CCM</title>
...
-->
 <!-- Modal Contact -->
				...
				<!-- / Modal Volumes logiques -->
					...
					<!-- Modal logiciels -->
				<div id="myModalLogiciels" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title">Modification de Machine virtuelle</span>
					</div>
					<div class="panel-body">
						<f:form modelAttribute="machineVirtuelle" action="/config/admin/add/saveMachineVirtuelle" methode="post" enctype="multipart/form-data" class="form-horizontal" id="jq-validation-form">
						...
						<div class="profile-content">
							...
							<div class="form-group">
								<label for="jq-validation-email" class="col-sm-3 control-label">RAM</label>
								<div class="col-sm-9">
				...
				<div class="form-group ">
					<label for="jq-validation-email" class="col-sm-3 control-label">Date de mise en production</label>
						...
						<div class=" col-sm-9" >
							...
							<div class="form-group">
								...
								<div class="tab-pane fade widget-logiciels" id="profile-tabs-logiciels">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableLogiciel">
											<c:forEach items="${machineVirtuelle.logicielEtApplication}" var="l">
												<tr class="gradeA" id="tr_logiciel_${l.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckla" name="ckLogiciel" value="${l.id }"></td>
													<td>${l.nom }</td>
													<td>${l.statut }</td>
													<td>${l.criticite }</td>
									...
									<div class="form-group">
										<div class="col-sm-offset-3 col-sm-1">
											<button type="button" class="btn btn-warning btn-flat" id="suppla">Retirer !</button>
										</div>
										
										
										<div class="col-sm-offset-1 col-sm-7">
											<button type="button" class="btn btn-success btn-flat" data-toggle="modal" data-target="#myModalLogiciels">Ajouter des Logiciel</button>
										</div>
								...
								<div class="tab-pane fade widget-contacts" id="profile-tabs-contacts">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContact">
											<c:forEach items="${machineVirtuelle.contacts}" var="contact">
												<tr class="gradeA" id="tr_contact_${contact.id }">
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tabledocument">
												<c:forEach items="${machineVirtuelle.documents}" var="doc">
													<tr class="gradeA" id="tr_doc_${doc.id }">
								...
								<div class="tab-pane fade widget-solutions" id="profile-tabs-solutions">
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<c:forEach items="${machineVirtuelle.solutionApplicative}" var="sa">
												<tr class="gradeA" id="tr_SolutionApplicative_${sa.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckSol" name="ckSolutionApplicative" value="${sa.id }"></td>
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableinterfacereseau">
												<c:forEach items="${machineVirtuelle.intefaceReseau}" var="ir">
												<tr class="gradeA" id="tr_ir_${ir.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckir" name="ckInterfacesReseaux" value="${ir.id }"></td>
													<td><a href="<c:url value="/config/view/logique?id=${ir.id }" />">${ir.nom }</a></td>
													<td>${ir.adresseIp }</td>
													<td>${ir.adresseMac }</td>
													<td>${ir.commentaire }</td>
													<td>${ir.passerelle }</td>
								...
								<div class="tab-pane fade widget-volumes_logiques" id="profile-tabs-volumes_logiques">
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableVolumesLogiques">
												<c:forEach items="${machineVirtuelle.volumelogique}" var="vl">
												<tr class="gradeA" id="tr_vl_${vl.id }">
								...
								<div class="tab-pane fade widget-contrats" id="profile-tabs-contrats">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContrat">
											<c:forEach items="${machineVirtuelle.contrats}" var="contrat">
												<tr class="gradeA" id="tr_contrat_${contrat.id }">
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-1">
									<button type="reset" class="btn btn-lg btn-danger btn-flat" onclick="location.href='<c:url value="/config/view/machineVirtuelle?id=${machineVirtuelle.id }" />'">Annuler</button>
								</div>
...
<script type="text/javascript">
	init.push(function () {
	    ...
	    document.getElementById("addla").onclick = function () {
	    	var chkArray = [];
	    	
	    	$(".ckla:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_logiciel_".concat($(this).val()));
		    	$( "#tableLogiciel" ).append(tr);
		    	//this.checked = false;
	    	});
	    	
	    };
	    
	    document.getElementById("suppla").onclick = function () {
			var chkArray = [];
	    	
	    	$(".ckla:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_logiciel_".concat($(this).val()));
		    	$( "#tableLogicielpopup" ).append(tr);
                this.checked = false;
	    	});
	    };
