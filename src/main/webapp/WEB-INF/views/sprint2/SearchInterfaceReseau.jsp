...
<head>
	...
	<title>Recherche Intérface Réseau - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Interfaces Réseaux</h1>
			<a href="<c:url value="/config/admin/add/interfacereseau"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Interface Réseau</a>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="interfacereseau" class="search-form bg-primary">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
												...
												<c:set var="string2" value="${fn:substring(string1, 29,40)}" />
										<tr class="gradeA">
											<td>
												<c:if test="${string2 == 'Fibre'}">
													<a href="<c:url value="/config/view/fibre?id=${ci.id }" />">${ci.nom }</a>
												</c:if>
												<c:if test="${string2 == 'Logique'}">
													<a href="<c:url value="/config/view/logique?id=${ci.id }" />">${ci.nom }</a>
												</c:if>
												
												<c:if test="${string2 == 'Physique'}">
													<a href="<c:url value="/config/view/physique?id=${ci.id }" />">${ci.nom }</a>
												</c:if>
											</td>
											<td>${ci.vitesse }</td>
											<td>
