...
<head>
	...
	<title>Recherche Volume Logique - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Volume Logique</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/volumelogique"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Volume Logique</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="volumelogique" class="search-form bg-primary">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/volumelogique?id=${ci.id }" />">${ci.nom }</a></td>
											<td>${ci.lunId }</td>
											<td>${ci.niveauRaid }</td>
											<td>${ci.taille }</td>
											<td><a href="<c:url value="/config/view/systemedestockage?id=${ci.systemedestockage.id }" />">${ci.systemedestockage.nom }</a></td>
