...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Modification Serveur Web - ITIL-CCM</title>
  ...
  <!-- Modal Contact -->
				...
				<div id="myModalApplicationWeb" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-default" data-dismiss="modal">Retour</button>
								<button type="button" class="btn btn-primary" id="addApp">Ajouter</button>
							</div>
						</div> <!-- / .modal-content -->
					</div> <!-- / .modal-dialog -->
				</div> <!-- /.modal -->
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title">Modification de Serveur Web</span>
					</div>
					<div class="panel-body">
						<f:form modelAttribute="serveurWeb" action="/config/admin/add/saveServeurWeb" methode="post" enctype="multipart/form-data" class="form-horizontal" id="jq-validation-form">
							...
							<div class="form-group required">
								...
								<div class="col-sm-9">
									...
									<c:if test="${serveurWeb.serveur.id != null }">
										<option value="serv_${serveurWeb.serveur.id }">${serveurWeb.serveur.nom }</option>
									</c:if>
									<c:if test="${serveurWeb.machineVirtuelle.id != null }">
										<option value="mach_${serveurWeb.machineVirtuelle.id }">${serveurWeb.machineVirtuelle.nom }</option>
									</c:if>
									<option value=""></option>
									...
									<c:if test="${error == true}">
												<div class="help-block">Choisissez un Système</div>
											</c:if>
								</div>
							</div>
							<div class="form-group">
				...
				<div class="form-group ">
					<label for="jq-validation-email" class="col-sm-3 control-label">Date de mise en production</label>
						...
						<div class=" col-sm-9" >
								...
								<div class="tab-pane fade widget-contacts" id="profile-tabs-contacts">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContact">
											<c:forEach items="${serveurWeb.contacts}" var="contact">
												<tr class="gradeA" id="tr_contact_${contact.id }">
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tabledocument">
													<c:forEach items="${serveurWeb.documents}" var="doc">
													<tr class="gradeA" id="tr_doc_${doc.id }">
								...
								<div class="tab-pane fade widget-solutions" id="profile-tabs-solutions">
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableSolutionApplicative">
												<c:forEach items="${serveurWeb.solutionApplicative}" var="sa">
												<tr class="gradeA" id="tr_SolutionApplicative_${sa.id }">
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableApplicationWeb">
												<c:forEach items="${serveurWeb.applicationWeb}" var="aw">
												<tr class="gradeA" id="tr_ApplicationWeb_${aw.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckApp" name="ckApplicationWeb" value="${aw.id }"></td>
													<td>${aw.nom }</td>
													<td>${aw.url }</td>
													<td>${aw.criticite}</td>
								...
								<div class="table-primary">
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContrat">
												<c:forEach items="${serveurWeb.contrats}" var="contrat">
												<tr class="gradeA" id="tr_contrat_${contrat.id }">
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-1">
									<button type="reset" class="btn btn-lg btn-danger btn-flat" onclick="location.href='<c:url value="/config/view/serveurWeb?id=${serveurWeb.id }" />'">Annuler</button>
								</div>
