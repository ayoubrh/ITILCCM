...
<head>
	...
	<title>Recherche Contact - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Contacts</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/contact"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Contact</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="contact" class="search-form bg-primary">
				<div class="input-group input-group-lg">
					<span class="input-group-addon no-background"><i class="fa fa-search"></i></span>
					<input type="text" name="c" class="form-control" value="${contact }" placeholder="Entrez le nom à rechercher...">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/contact?id=${ci.id }" />">${ci.nom }</a></td>
											<td>${ci.prenom }</td>
											<td>${ci.organisme }</td>
											<td>${ci.statut }</td>
											<td><a href="<c:url value="/config/view/lieu?id=${ci.lieu.id }" />">${ci.lieu.nom }</a></td>
											<td>${ci.fonction }</td>
											<td>${ci.numeroD_employe }</td>
											<td>${ci.email }</td>
											<td>${ci.telephoneFixe }</td>
											<td>${ci.telephoneMobile }</td>
										</tr>
