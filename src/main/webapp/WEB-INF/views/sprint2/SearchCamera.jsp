...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Recherche Caméras - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Caméras</h1>
			<a href="<c:url value="/config/admin/add/camera"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouvelle Caméra</a>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="camera" class="search-form bg-primary">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/camera?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/dvr?id=${ci.dvr.id }" />">${ci.dvr.nom }</a></td>
											<td>${ci.position }</td>
											<td>${ci.statut }</td>
											<td>${ci.marque }</td>
											<td>${ci.modele }</td>
