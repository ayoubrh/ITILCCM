...
<head>
	...
	<title>Recherche Systèmes de Stockage - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Systèmes de Stockage</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/systemedestockage"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Système de Stockage</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="systemedestockage" class="search-form bg-primary">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/systemedestockage?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/rack?id=${ci.rack.id }" />">${ci.rack.nom }</a></td>
											<td><a href="<c:url value="/config/view/chassis?id=${ci.chassis.id }" />">${ci.chassis.nom }</a></td>
											<td>${ci.statut }</td>
											<td>${ci.marque }</td>
											<td>${ci.modele }</td>
