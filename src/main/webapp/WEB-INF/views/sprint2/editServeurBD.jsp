...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Modification Serveur de base de données - ITIL-CCM</title>
...
-->
  ...
  <!-- Modal Contact -->
				...
				<!-- / Modal Solutions Applicatives -->	
		<!-- Modal Instances BD -->
				<div id="myModalInstancesBD" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
								<h4 class="modal-title" id="myModalLabel">Ajout Instances de Base de Données</h4>
							</div>
							<div class="modal-body">
								<div class="table-warning">
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableInstanceBDpopup">
											<c:forEach items="${instanceBD}" var="ibd">
												<tr class="gradeA" id="tr_InstanceBD_${ibd.id }">
													<td class="supchekbox"><input type="checkbox" class="ckInstBD" name="ckInstanceBD" value="${ibd.id }"></td>
													<td>${ibd.nom }</td>
													<td>${ibd.criticite}</td>
													<td>${ibd.dateDeMiseEnProduction }</td>
													<td>${ibd.description }</td>
												</tr>
											</c:forEach>
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-default" data-dismiss="modal">Retour</button>
								<button type="button" class="btn btn-primary" id="addInstBD">Ajouter</button>
							</div>
						</div> <!-- / .modal-content -->
					</div> <!-- / .modal-dialog -->
				</div> <!-- /.modal -->
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title">Modification de Serveur de base de données</span>
					</div>
					<div class="panel-body">
						<f:form modelAttribute="serveurBD" action="/config/admin/add/saveServeurBD" methode="post" enctype="multipart/form-data" class="form-horizontal" id="jq-validation-form">
						...
						<div class="profile-content">
							...
							<div class="form-group required">
								...
								<div class="col-sm-9">
									...
									<c:if test="${serveurBD.serveur.id != null }">
										<option value="serv_${serveurBD.serveur.id }">${serveurBD.serveur.nom }</option>
									</c:if>
									<c:if test="${serveurBD.machineVirtuelle.id != null }">
										<option value="mach_${serveurBD.machineVirtuelle.id }">${serveurBD.machineVirtuelle.nom }</option>
									</c:if>
									<option value=""></option>
				...
				<div class="form-group ">
					<label for="jq-validation-email" class="col-sm-3 control-label">Date de mise en production</label>
						...
						<div class=" col-sm-9" >
							...
							<div class="form-group">
								...
								<div class="tab-pane fade widget-contacts" id="profile-tabs-contacts">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContact">
											<c:forEach items="${serveurBD.contacts}" var="contact">
												<tr class="gradeA" id="tr_contact_${contact.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckContact" name="ckContacts" value="${contact.id }"></td>
													<td>${contact.nom } ${contact.prenom }</td>
													<td>${contact.statut }</td>
													<td>${contact.organisme }</td>
													<td>${contact.email }</td>
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tabledocument">
													<c:forEach items="${serveurBD.documents}" var="doc">
													<tr class="gradeA" id="tr_doc_${doc.id }">
								...
								<div class="tab-pane fade widget-solutions" id="profile-tabs-solutions">
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableSolutionApplicative">
												<c:forEach items="${serveurBD.solutionApplicative}" var="sa">
												<tr class="gradeA" id="tr_SolutionApplicative_${sa.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked"  class="ckSol" name="chSolutionApplicative" value="${sa.id }"></td>
													<td>${sa.nom }</td>
													<td>${sa.statut }</td>
													<td>${sa.criticite}</td>
								...
								<div class="tab-pane fade widget-instance" id="profile-tabs-instance">
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableInstanceBD">
											<c:forEach items="${serveurBD.instanceDeBasseDeDonnes}" var="ibd">
												<tr class="gradeA" id="tr_InstanceBD_${ibd.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckInstBD" name="ckInstanceBD" value="${ibd.id }"></td>
													<td>${ibd.nom }</td>
													<td>${ibd.criticite}</td>
									...
									<div class="form-group">
										<div class="col-sm-offset-3 col-sm-1">
											<button type="button" class="btn btn-warning btn-flat" id="suppInstBD">Retirer !</button>
										</div>
										
										<div class="col-sm-offset-1 col-sm-7">
											<button type="button" class="btn btn-success btn-flat" data-toggle="modal" data-target="#myModalInstancesBD">Ajouter des Instances de Base de Données</button>
										</div>
								...
								<div class="tab-pane fade widget-contrats" id="profile-tabs-contrats">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContrat">
												<c:forEach items="${serveurBD.contrats}" var="contrat">
												<tr class="gradeA" id="tr_contrat_${contrat.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckContrat" name="ckContrats" value="${contrat.id }"></td>
													<td>${contrat.nom }</td>
													<td>${contrat.typeDeContrat }</td>
													<td>${contrat.client }</td>
													<td>${contrat.description }</td>
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-1">
									<button type="reset" class="btn btn-lg btn-danger btn-flat" onclick="location.href='<c:url value="/config/view/serveurBD?id=${serveurBD.id }" />'">Annuler</button>
								</div>
...
<script type="text/javascript">
	init.push(function () {
	    ...
	    document.getElementById("addInstBD").onclick = function () {
	    	var chkArray = [];
	    	
	    	$(".ckInstBD:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_InstanceBD_".concat($(this).val()));
		    	$( "#tableInstanceBD").append(tr);
		    	//this.checked = false;
	    	});
	    	
	    };
	    
	    document.getElementById("suppInstBD").onclick = function () {
			var chkArray = [];
	    	
	    	$(".ckInstBD:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_InstanceBD_".concat($(this).val()));
		    	$( "#tableInstanceBDpopup").append(tr);
                this.checked = false;
	    	});
	    };
	});
