...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Modification Rack - ITIL-CCM</title>
	...
	<!-- Modal chassis -->
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title">Modification Rack</span>
					</div>
					<div class="panel-body">
						<f:form modelAttribute="rack" action="/config/admin/add/saveRack" methode="post" class="form-horizontal" id="jq-validation-form">
						...
						<div class="profile-content">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tablechassis">
											<c:forEach items="${rack.chassis}" var="chas">
												<tr class="gradeA" id="tr_chassis_${chas.id }">
													<td class="supchekbox"><input type="checkbox" class="ckChassis" checked="checked" name="chchassis" value="${chas.id }"></td>
													<td>${chas.nom }</td>
													<td>${chas.statut }</td>
													<td>${chas.criticite }</td>
													<td>${chas.numeroDeSerie }</td>
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableMatriel">
											<c:forEach items="${rack.infrastructure}" var="mat">
												<tr class="gradeA" id="tr_mat_Mat_${mat.id }">
													<td class="supchekbox"><input type="checkbox" class="ckMat" checked="checked" name="chMateriel" value="Mat_${mat.id }"></td>
													<td>${mat.nom }</td>
													<td>${mat.statut }</td>
													<td>${mat.criticite }</td>
													<td>${mat.numeroDeSerie }</td>
											...
											<c:forEach items="${rack.pduElectrique}" var="pdu">
												<tr class="gradeA" id="tr_mat_PDU_${pdu.id }">
													<td class="supchekbox"><input type="checkbox" class="ckMat" checked="checked" name="chMateriel" value="PDU_${pdu.id }"></td>
													<td>${pdu.nom }</td>
													<td>${pdu.statut }</td>
													<td>${pdu.criticite }</td>
													<td>${pdu.numeroDeSerie }</td>
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-1">
									<button type="reset" class="btn btn-lg btn-danger btn-flat" onclick="location.href='<c:url value="/config/view/rack?id=${rack.id }" />'">Annuler</button>
								</div>
