...
<head>
	...
	<title>Recherche Application Web - ITIL-CCM</title>
...
-->
		...
		<div class="page-header">
			<h1 class="col-md-9"><i class="fa fa-search page-header-icon"></i>&nbsp;&nbsp;Recherche des Applications Web</h1>
			<s:authorize ifAnyGranted="ROLE_ADMIN">
			<a href="<c:url value="/config/admin/add/applicationWeb"/>" class="btn btn-success"><i class="fa"></i>&nbsp;Créer nouveau Application Web</a>
			</s:authorize>
		</div> <!-- / .page-header -->
		...
		<div class="panel search-panel">
			...
			<!-- Search form -->
			<form action="applicationWeb" class="search-form bg-primary">
				<div class="input-group input-group-lg">
					<span class="input-group-addon no-background"><i class="fa fa-search"></i></span>
					<input type="text" name="aw" class="form-control" value="${applicationWeb }" placeholder="Entrez le nom à chercher...">
					...
					<table cellpadding="0" cellspacing="0" border="0" class="table table-primary table-striped table-bordered" id="jq-datatables-example">
								...
								<tbody>
									<c:forEach items="${cis}" var="ci">
										<tr class="gradeA">
											<td><a href="<c:url value="/config/view/applicationWeb?id=${ci.id }" />">${ci.nom }</a></td>
											<td><a href="<c:url value="/config/view/serveurWeb?id=${ci.serveurWeb.id }" />">${ci.serveurWeb.nom}</a></td>
											<td>${ci.url }</td>
											<td>${ci.criticite }</td>
