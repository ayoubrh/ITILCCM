...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<title>Modification Middleware - ITIL-CCM</title>
...
-->
  ...
  <!-- Modal Contact -->
				...
				<!-- Modal Instances Middleware -->
				<div id="myModalInstancesMiddleware" class="modal fade" tabindex="-1" role="dialog" style="display: none;">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
								<h4 class="modal-title" id="myModalLabel">Ajout Instances Middleware</h4>
							</div>
							<div class="modal-body">
								<div class="table-warning">
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-default" data-dismiss="modal">Retour</button>
								<button type="button" class="btn btn-primary" id="addInstM">Ajouter</button>
							</div>
						</div> <!-- / .modal-content -->
					</div> <!-- / .modal-dialog -->
				</div> <!-- /.modal -->
		...
		<div class="panel">
					<div class="panel-heading">
						<span class="panel-title">Modification de Middleware</span>
					</div>
					<div class="panel-body">
						<f:form modelAttribute="middleware" action="/config/admin/add/saveMiddleware" methode="post" enctype="multipart/form-data" class="form-horizontal" id="jq-validation-form">
						...
						<div class="profile-content">
							...
							<div class="form-group required">
								...
								<div class="col-sm-9">
									...
									<c:if test="${middleware.serveur.id != null }">
										<option value="serv_${middleware.serveur.id }">${middleware.serveur.nom }</option>
									</c:if>
									<c:if test="${middleware.machineVirtuelle.id != null }">
										<option value="mach_${middleware.machineVirtuelle.id }">${middleware.machineVirtuelle.nom }</option>
									</c:if>
									<option value=""></option>
				...
				<div class="form-group ">
					<label for="jq-validation-email" class="col-sm-3 control-label">Date de mise en production</label>
						...
						<div class=" col-sm-9" >
								...
								<div class="tab-pane fade widget-contacts" id="profile-tabs-contacts">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContact">
											<c:forEach items="${middleware.contacts}" var="contact">
												<tr class="gradeA" id="tr_contact_${contact.id }">
									...
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tabledocument">
													<c:forEach items="${middleware.documents}" var="doc">
													<tr class="gradeA" id="tr_doc_${doc.id }">
								...
								<div class="tab-pane fade widget-solutions" id="profile-tabs-solutions">
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableSolutionApplicative">
												<c:forEach items="${middleware.solutionApplicative}" var="sa">
												<tr class="gradeA" id="tr_SolutionApplicative_${sa.id }">
								...
								<div class="tab-pane fade widget-instance" id="profile-tabs-instance">
									<div class="table-primary">
										<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
											...
											<tbody id="tableInstanceMiddleware">
												<c:forEach items="${middleware.instanceMiddleware}" var="im">
												<tr class="gradeA" id="tr_InstanceMiddleware_${im.id }">
													<td class="supchekbox"><input type="checkbox" checked="checked" class="ckInstM" name="ckInstanceMiddleware" value="${im.id }"></td>
													<td>${im.nom }</td>
													<td>${im.criticite}</td>
									...
									<div class="form-group">
										<div class="col-sm-offset-3 col-sm-1">
											<button type="button" class="btn btn-warning btn-flat" id="suppInstM">Retirer !</button>
										</div>
										
										<div class="col-sm-offset-1 col-sm-7">
											<button type="button" class="btn btn-success btn-flat" data-toggle="modal" data-target="#myModalInstancesMiddleware">Ajouter des Instances Middleware</button>
										</div>
								...
								<div class="tab-pane fade widget-contrats" id="profile-tabs-contrats">
									...
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered jq-datatables-example">
										...
										<tbody id="tableContrat">
												<c:forEach items="${middleware.contrats}" var="contrat">
												<tr class="gradeA" id="tr_contrat_${contrat.id }">
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-1">
									<button type="reset" class="btn btn-lg btn-danger btn-flat" onclick="location.href='<c:url value="/config/view/middleware?id=${middleware.id }" />'">Annuler</button>
								</div>
...
<script type="text/javascript">
	init.push(function () {
	    ...
	    document.getElementById("addInstM").onclick = function () {
	    	var chkArray = [];
	    	
	    	$(".ckInstM:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_InstanceMiddleware_".concat($(this).val()));
		    	$( "#tableInstanceMiddleware").append(tr);
		    	//this.checked = false;
	    	});
	    	
	    };
	    
	    document.getElementById("suppInstM").onclick = function () {
			var chkArray = [];
	    	
	    	$(".ckInstM:checked").each(function() {
	    		chkArray.push($(this).val());
	    		var tr = document.getElementById("tr_InstanceMiddleware_".concat($(this).val()));
		    	$( "#tableInstanceMiddlewarepopup").append(tr);
                this.checked = false;
	    	});
	    };
	});
